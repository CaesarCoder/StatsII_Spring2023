...
lm3 <- lm(general~cmc_chair_connection_current+combat_post_1949+
            college+participated_long_march+commissar+minority+parent_CCP_leader+rural, data=bio, subset=post_deng==1)
...
lm6 <- lm(cmc~cmc_chair_connection_current+combat_post_1949
          +college+participated_long_march+commissar+minority+parent_CCP_leader+rural, data=bio, subset=post_deng==1)
...
stargazer(lm1, lm2, lm3, lm4, lm5, lm6, type = "latex", 
          ...
          omit.stat=c("f", "ser"),
          dep.var.labels = c("Promoted to General", "Promoted to CMC"),
          title = "Post-Deng Officers Only: Promotion to full general-level 
          position and to the CCP Central Military Commission",
          star.char = c("+", "*", "**"),
          notes.append=FALSE,
          notes = c("$^{+}$p$<$0.1; $^{*}$p$<$0.05; $^{**}$p$<$0.01}"),
          covariate.labels = c("Career Tie to Paramount Leader", 
                               "Combat Experience, Post-1949", "College-Level Education", 
                               "Long March Participant", "Political Commissar Experience", 
                               "Ethnic Minority", "Princeling", "Rural Birth"),
          out = "Table_A11.tex")
...
glm1 <- glm(general~cmc_chair_connection_current, data=bio, subset=post_deng==1, family=binomial(link="logit"))
glm2 <- glm(general~combat_post_1949, data=bio, subset=post_deng==1, family=binomial(link="logit"))
glm3 <- glm(general~cmc_chair_connection_current+combat_post_1949+college+participated_long_march+
             commissar+minority+parent_CCP_leader+rural, data=bio, subset=post_deng==1, family=binomial(link="logit"))
glm4 <- glm(cmc~cmc_chair_connection_current, data=bio, subset=post_deng==1, family=binomial(link="logit"))
glm5 <- glm(cmc~combat_post_1949, data=bio, subset=post_deng==1, family=binomial(link="logit"))
glm6 <- glm(cmc~cmc_chair_connection_current+combat_post_1949+college+participated_long_march+
             commissar+minority+parent_CCP_leader+rural, data=bio, subset=post_deng==1, family=binomial(link="logit"))

rob.fit1        <- coeftest(glm1, function(x) vcovHC(x, type="HC3"))
rob.fit2        <- coeftest(glm2, function(x) vcovHC(x, type="HC3"))
rob.fit3        <- coeftest(glm3, function(x) vcovHC(x, type="HC3"))
rob.fit4        <- coeftest(glm4, function(x) vcovHC(x, type="HC3"))
rob.fit5        <- coeftest(glm5, function(x) vcovHC(x, type="HC3"))
rob.fit6        <- coeftest(glm6, function(x) vcovHC(x, type="HC3"))

stargazer(glm1, glm2, glm3, glm4, glm5, glm6, type = "latex", 
          se = list(rob.fit1[,"Std. Error"], rob.fit2[,"Std. Error"],
                    rob.fit3[,"Std. Error"], rob.fit4[,"Std. Error"],
                    rob.fit5[,"Std. Error"], rob.fit6[,"Std. Error"]), 
          omit=c("cohort_decade"),
          ...
          omit.stat=c("f", "ser"),
          dep.var.labels = c("Promoted to General", "Promoted to CMC"),
          title = "glm, Post-Deng Officers Only: Promotion to full general-level
          position and to the CCP Central Military Commission",
          star.char = c("+", "*", "**"),
          notes.append=FALSE,
          notes = c("$^{+}$p$<$0.1; $^{*}$p$<$0.05; $^{**}$p$<$0.01}"),
...
stargazer(glm1, title = "general promotion, foreign threat")


glm2 <- plm(general_varying~combat_post_1949*foreign2+age+cmc_connection_varying*foreign2+college*foreign2+participated_long_march*foreign2+commissar*foreign2+combat_post_1949*foreign2+parent_CCP_leader*foreign2+rural*foreign2+as.factor(year),  data=panel_year, family=binomial(link="logit"))



glm3 <- plm(general_varying~combat_post_1949*foreign2+age+cmc_connection_varying*foreign2+college*foreign2+participated_long_march*foreign2+commissar*foreign2+combat_post_1949*foreign2+parent_CCP_leader*foreign2+rural*foreign2+as.factor(year)*cohort_decade,  data=panel_year, family=binomial(link="logit"))



glm4 <- plm(general_varying~combat_post_1949*foreign2+age+cmc_connection_varying*foreign2+college*foreign2+participated_long_march*foreign2+commissar*foreign2+combat_post_1949*foreign2+parent_CCP_leader*foreign2+rural*foreign2+as.factor(year)*as.factor(birth_year),  data=panel_year, family=binomial(link="logit"))





stargazer(glm1, glm2, glm3, glm4, type = "latex", 
          se = list(cluster.fit1[,"Std. Error"], cluster.fit2[,"Std. Error"],
                    cluster.fit3[,"Std. Error"], cluster.fit4[,"Std. Error"]), 
          omit=c("cohort_decade", "year", "Constant","age" , "college", "participated_long_march", "commissar", "cmc_connection_varying", "minority", "participated_long_march", "rural", "parent_CCP_leader"),
          omit.stat=c("f", "ser"),
